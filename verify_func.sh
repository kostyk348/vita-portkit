...
#   ./verify_func.sh <file.c> <func> <r0> <r1> <expected_exit> [func_index]
...
FUNC="${2:?}"; A0="${3:?}"; A1="${4:?}"; EXPECT="${5:?}"; IDX="${6:-0}"
...
python3 - "$W" "$A0" "$A1" "$IDX" <<'PY'
import sys, os
w, a0, a1, idx = sys.argv[1], sys.argv[2], sys.argv[3], int(sys.argv[4])
